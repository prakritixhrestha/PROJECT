...
class AdminAccessMiddleware:
    ...
    def __call__(self, request):
        ...
        if (request.path.startswith('/admin/') or request.path.startswith('/admin-custom/')) and not request.path.startswith('/django-admin/'):
            ...
            # 0. Avoid infinite redirect if already on login page or accessing static files
            ...
            # 2. If user IS authenticated but NOT staff, redirect to login so they can switch accounts
